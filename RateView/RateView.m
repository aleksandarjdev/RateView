...
//  Youth_Hostel
//
//  Created by Aleksandar Jovanov on 5/31/25.
//  Copyright © 2025 World Hostel Group Corporation. All rights reserved.
//
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        ...
        // Set initial rating after view is properly initialized
        self.rating = 0.0f;
    }
    return self;
}
...
{
    if (!starNormalColor) {
        starNormalColor = DefaultStarNormalColor;
    }
...
{
    if (!starFillColor) {
        starFillColor = DefaultStarFillColor;
    }
...
{
    if (!starBorderColor) {
        starBorderColor = DefaultStarBorderColor;
    }
...
{
    if (starSize <= 0.0f) {
        starSize = DefaultStarSize;  // Use default size if invalid
    }
    _starSize = starSize;
    [self updateRateView];
...
{
    if(_canRate && _starSize > 0.0f) {  // Add _starSize check
        CGPoint location = [[touches anyObject] locationInView:self];
        if (CGRectContainsPoint(self.bounds, location)) {  // Check if point is within bounds
            RVLog(@"Location: %@", NSStringFromCGPoint(location));
            // Compute location
            float x = location.x;
            if(x < 0.0f)
                x = 0.0f;
                ...
                if (div > 0.0f) {  // Prevent division by zero
                    x = (x / div) + self.step;
                    x = div * (int)x;
                }
            }
